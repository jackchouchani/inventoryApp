...
import { Item, Container, getContainers, Category } from '../../src/database/database';
import * as Print from 'expo-print';
import * as Sharing from 'expo-sharing';
import QRCode from 'react-native-qrcode-svg';
import * as FileSystem from 'expo-file-system';
import { Buffer } from 'buffer';
import { HiddenQRCode } from '../../src/components/HiddenQRCode';
...
const LabelScreen = () => {
  ...
  const qrRef = React.createRef<QRCode>();
  ...
  const getQRCodeBase64Local = (value: string): Promise<string> => {
    return new Promise((resolve) => {
      if (Platform.OS === 'web') {
        // Pour le web, on utilise directement le composant QRCode
        setQrCaptureResolve(() => resolve);
        setQrCaptureValue(value);
      } else {
        // Pour mobile, utilisez un composant QRCode avec une référence
        const qrCodeSvg = (
          <QRCode
            value={value}
            getRef={(ref) => {
        ...
        // Rendu du QR code pour obtenir l'image
        // Vous devrez peut-être utiliser un renderer pour cela
      }
    });
  };
  ...
  const generateLabels = async () => {
    try {
      ...
      if (Platform.OS === 'web') {
        try {
          // Importer jspdf d'une manière différente pour éviter les problèmes de bundling
        ...
        Alert.alert('Erreur', 'Impossible de générer le PDF sur le web');
        return;
      }
    } else {
